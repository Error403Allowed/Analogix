...
export const ToolSchemas = {
  ...
  get_documents: {
    name: 'get_documents',
    description: 'Retrieve documents from the user\'s workspace.',
    parameters: z.object({
  ...
  search_curriculum: {
    ...
    description: 'Search the Australian curriculum for learning outcomes, topics, and content descriptions by subject, grade, and state.',
    parameters: z.object({
      query: z.string().describe('The search query for curriculum content'),
      subject: z.string().optional().describe('Filter by subject (e.g. Mathematics, English, Science)'),
      grade: z.string().optional().describe('Filter by grade/year level (e.g. 7, 8, 9, 10, 11, 12)'),
      state: z.string().optional().describe('Filter by state (e.g. NSW, VIC, QLD, WA, or NATIONAL for ACARA)'),
    }),
  },
};
