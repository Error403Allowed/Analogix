...
export async function POST(request) {
    ...
    try {
        ...
        if (file.size > MAX_FILE_SIZE) {
            return NextResponse.json({ error: `File is too large (max 50 MB). Your file is ${(file.size / 1024 / 1024).toFixed(1)} MB.` }, { status: 413 });
        }
        ...
        if (!text.trim()) {
            return NextResponse.json({ error: "No content could be extracted from this file. Try a .txt, .pdf, or image file." }, { status: 422 });
        }
