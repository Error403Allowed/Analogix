...
export const flashcardStore = {
    ...
    getSets: async () => {
        ...
        if (error) {
            console.warn("[flashcardStore] getSets failed:", error);
            return [];
        }
    ...
    createSet: async (subjectId, name) => {
        ...
        const { data, error } = await supabase
            ...
            .single();
        if (error) {
            console.warn("[flashcardStore] createSet failed:", error);
            return null;
        }
        broadcastChange();
        return toSet(data);
    ...
    getBySet: async (setId) => {
        ...
        const { data, error } = await supabase
            ...
            .order("created_at", { ascending: false });
        if (error) {
            console.warn("[flashcardStore] getBySet failed:", error);
            return [];
        }
        return (data ?? []).map(toCard);
    },
    ...
    add: async (cards) => {
        ...
        const newCards = uniqueCards.map(c => ({
            ...c,
        ...
        const { error } = await supabase.from("flashcards").insert(newCards.map(c => ({
            id: c.id,
            user_id: user.id,
            set_id: c.setId || null,
