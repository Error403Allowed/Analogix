...
export function EventChip({ event, allTypes, onClick }: { event: AppEvent; allTypes: Record<string,{color:string;label:string;icon:string}>; onClick?: () => void }) {
  ...
  return (
    <button onClick={onClick} title={event.location ? `${event.title} · ${event.location}` : event.title}
      ...
      <span className="truncate">{event.title}{event.location ? ` · ${event.location}` : ""}</span>
    </button>
  );
}
