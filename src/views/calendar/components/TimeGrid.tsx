...
export function TimeGrid({ days, events, allTypes, now, hourHeight = HOUR_H, visibleHours = HOURS, defaultDurationMinutes = DEFAULT_EVENT_DURATION_MINUTES, onDelete, onSelect, onCreateSelection, onUpdateEvent, onExpandDay }: {
  days: Date[]; events: AppEvent[];
  allTypes: Record<string,{color:string;label:string;icon:string}>;
  now: Date; hourHeight?: number; visibleHours?: number[];
  defaultDurationMinutes?: number;
  onDelete: (id: string) => void; onSelect: (e: AppEvent) => void;
  ...
  const endHour = (visibleHours[visibleHours.length - 1] ?? 23) + 1;
  const rangeStartMin = startHour * 60;
  const rangeEndMin = endHour * 60;

  const yForMinutes = useCallback((minutes: number) => (
    TOP_PAD + (minutes - rangeStartMin) * (HH / 60)
  ), [TOP_PAD, rangeStartMin, HH]);

  const clampToVisible = useCallback((minutes: number, durationMinutes = 0) => (
    Math.min(Math.max(minutes, rangeStartMin), Math.max(rangeEndMin - durationMinutes, rangeStartMin))
  ), [rangeStartMin, rangeEndMin]);

  useEffect(() => {
    const anchorMin = nowMinutes >= rangeStartMin && nowMinutes <= rangeEndMin ? nowMinutes : rangeStartMin;
    containerRef.current?.scrollTo({ top: Math.max(0, anchorMin * (HH / 60) - 200), behavior: "smooth" });
  }, [nowMinutes, rangeStartMin, rangeEndMin, HH]);
  ...
  const getPointerSlot = useCallback((clientX: number, clientY: number) => {
    ...
    const relativeY = clientY - rect.top + container.scrollTop - TOP_PAD;

    return {
      dayIndex: Math.min(Math.max(Math.floor(relativeX / Math.max(dayWidth, 1)), 0), days.length - 1),
      minutes: snapMinutes(clampToVisible(rangeStartMin + relativeY / (HH / 60))),
    };
  }, [days.length, TOP_PAD, HH, rangeStartMin, clampToVisible]);
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (pointerEvent: PointerEvent) => {
      ...
      setInteraction((current) => {
        ...
        if (current.kind === "move") {
          const didDrag = current.didDrag || hasMovedEnough(pointerEvent.clientX, pointerEvent.clientY, current.startClientX, current.startClientY);
          if (!didDrag) return current;
          const nextStartMin = clampToVisible(snapMinutes(slot.minutes - current.pointerOffsetMin), current.durationMin);
        ...
        if (current.edge === "start") {
          const { startMin } = normalizeMinuteRange(Math.max(slot.minutes, rangeStartMin), current.previewEndMin);
        ...
        return { ...current, didDrag: true, previewEndMin: Math.min(endMin, rangeEndMin) };
      });
    };
    ...
    const handlePointerUp = (pointerEvent: PointerEvent) => {
      ...
      if (current.kind === "create") {
        const startMin = current.didDrag ? current.startMin : clampEventStartMinutes(current.anchorMin, defaultDurationMinutes);
        const endMin = current.didDrag ? current.endMin : Math.min(startMin + defaultDurationMinutes, 24 * 60);
      ...
      const sourceDurationMinutes = getEventDurationMinutes(sourceEvent, defaultDurationMinutes, getSiblingDayEvents(sourceEvent));
  ...
  }, [days, events, getPointerSlot, getSiblingDayEvents, interaction, onCreateSelection, onSelect, onUpdateEvent, defaultDurationMinutes, clampToVisible, rangeStartMin, rangeEndMin]);
  ...
  const startCreateInteraction = (dayIndex: number, pointerEvent: React.PointerEvent<HTMLDivElement>) => {
    ...
    setInteraction({
      ...
      startMin: slot.minutes, endMin: Math.min(slot.minutes + defaultDurationMinutes, 24 * 60),
  ...
  const startMoveInteraction = (event: AppEvent, pointerEvent: React.PointerEvent<HTMLDivElement>) => {
    ...
    const durationMinutes = getEventDurationMinutes(event, defaultDurationMinutes, getSiblingDayEvents(event));
    pointerEvent.preventDefault();
  ...
  const startResizeInteraction = (event: AppEvent, edge: "start" | "end", pointerEvent: React.PointerEvent<HTMLDivElement>) => {
    ...
    pointerEvent.stopPropagation();
    const startMinutes = getEventStartMinutes(event);
    const endMinutes = startMinutes + getEventDurationMinutes(event, defaultDurationMinutes, getSiblingDayEvents(event));
    setInteraction({
      ...
      <div className="w-14 shrink-0 relative bg-background z-10">
        ...
        {visibleHours.map(h => (
          <div key={h} className="relative" style={{ height: HH }}>
            <span className="absolute -top-2.5 right-2 text-[10px] font-semibold text-muted-foreground/70 tabular-nums select-none">
              {h === startHour ? "" : format(setHours(setMinutes(new Date(), 0), h), "h a")}
            </span>
          </div>
        ))}
      </div>
      <div ref={columnsRef} className="flex flex-1 relative">
        {showNow && nowMinutes >= rangeStartMin && nowMinutes <= rangeEndMin && (
          <div className="absolute left-0 right-0 z-20 flex items-center pointer-events-none"
            style={{ top: yForMinutes(nowMinutes) }}>
            <div className="w-2 h-2 rounded-full bg-red-500 ml-0.5 shrink-0" />
            <div className="flex-1 h-0.5 bg-red-500/70 rounded-full" />
          </div>
        )}
        {days.map((day, di) => {
          const allDayEvents = displayEvents.filter(e => isSameDay(new Date(e.date), day));
          const inRange = allDayEvents.filter(e => {
            const m = getEventStartMinutes(e);
            return m >= rangeStartMin && m < rangeEndMin;
          });
          const earlierCount = allDayEvents.filter(e => getEventStartMinutes(e) < rangeStartMin).length;
          const laterCount = allDayEvents.filter(e => getEventStartMinutes(e) >= rangeEndMin).length;
          const laid: any = layoutEvents(inRange, HH).map((item: any) => ({
            ...item,
            top: item.top - rangeStartMin * (HH / 60),
          }));
          const createPreview = interaction?.kind === "create" && interaction.dayIndex === di
            ? {
                startMin: interaction.didDrag ? interaction.startMin : clampEventStartMinutes(interaction.anchorMin, defaultDurationMinutes),
                endMin: interaction.didDrag ? interaction.endMin : Math.min(clampEventStartMinutes(interaction.anchorMin, defaultDurationMinutes) + defaultDurationMinutes, 24 * 60),
              }
            : null;
          return (
            <div key={di} className="relative flex-1 border-r border-border/40 last:border-r-0"
              ...
              <div style={{ height: TOP_PAD }} />
              {(earlierCount > 0) && (
                <button onClick={onExpandDay} title="Show earlier events"
                  className="absolute top-1 left-1 right-1 z-10 text-center text-[10px] font-bold text-primary bg-background/95 border border-primary/30 rounded-full py-0.5 shadow-sm hover:bg-primary/10 transition-colors">
                  ↑ {earlierCount} earlier
                </button>
              )}
              {visibleHours.map(h => (
                <div key={h} className="border-t border-border/20 hover:bg-primary/5 transition-colors cursor-pointer group relative" style={{ height: HH }}>
              ...
              {visibleHours.map(h => (
                <div key={`half-${h}`} className="absolute left-0 right-0 border-t border-border/10 pointer-events-none"
                  style={{ top: TOP_PAD + (h - startHour) * HH + HH / 2 }} />
              ))}
              {(laterCount > 0) && (
                <button onClick={onExpandDay} title="Show later events"
                  className="absolute bottom-1 left-1 right-1 z-10 text-center text-[10px] font-bold text-primary bg-background/95 border border-primary/30 rounded-full py-0.5 shadow-sm hover:bg-primary/10 transition-colors">
                  ↓ {laterCount} later
                </button>
              )}
              {createPreview && (
                <div className="absolute left-[3%] right-[3%] rounded-xl border border-dashed border-primary/50 bg-primary/15 pointer-events-none"
                  style={{ top: yForMinutes(createPreview.startMin), height: Math.max((createPreview.endMin - createPreview.startMin) * (HH / 60), 18) }}>
              ...
              {laid.map(({ event, top, height, col, totalCols, span }: { event: any; top: any; height: any; col: any; totalCols: any; span: any }) => (
                <div key={event.id} className="absolute left-0 right-0" style={{ top: top + TOP_PAD, height }}>
