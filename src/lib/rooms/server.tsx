...
export function roomPermissionsOf(row: any): RoomPermissions {
    const stored = typeof row?.permissions === "object" && row?.permissions ? row.permissions : {};
    return { ...DEFAULT_ROOM_PERMISSIONS, ...stored };
}
export function mapStudyRoom(row: any, viewerRole: string | null, isOwner: boolean) {
    return {
        ...
        permissions: isOwner ? ADMIN_ROOM_PERMISSIONS : roomPermissionsOf(row),
        timerState: row.timer_state === "running" || row.timer_state === "paused" ? row.timer_state : "idle",
...
export async function getRoomAccess(supabase: any, userId: string, roomId: string) {
    ...
    const viewerRole = (isOwner ? "host" : membership?.role);
    return {
        room,
        membership,
        isOwner,
...
export async function listRoomMessages(supabase: any, roomId: string, limit = 100) {
    ...
    return (data ?? []).map((row: any) => {
        ...
        return {
            ...
            messageType: row.message_type === "ai" || row.message_type === "system" ? row.message_type : "chat",
            content: String(row.content ?? ""),
            createdAt: String(row.created_at),
            metadata,
            name: row.message_type === "ai"
                ? "Analogix AI"
...
export async function getRoomState(roomId: string) {
    ...
    return {
        ...access,
        room: mapStudyRoom(access.room, access.viewerRole, access.isOwner),
        members,
        messages,
        canvas,
        sharedDocuments,
