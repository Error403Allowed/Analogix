...
export function formatContextForPrompt(context: AppContext): string {
  const parts: string[] = [];

  // @ts-ignore
  if (context._profileSummary) {
    parts.push(`About me: ${(context as any)._profileSummary}`);
  }
  ...
  if (context.subjects && context.subjects.length > 0) {
    parts.push(`My subjects: ${context.subjects.join(", ")}`);
  }

  if (context.recentDocuments && context.recentDocuments.length > 0) {
    parts.push(`Recent documents:\n${context.recentDocuments.map(d => `  - "${d.title}" (${d.subject})`).join("\n")}`);
  }

  if (context.upcomingEvents && context.upcomingEvents.length > 0) {
    parts.push(`Upcoming events:\n${context.upcomingEvents.map(e => `  - ${e.title} (${e.type}): ${e.date}`).join("\n")}`);
  }

  if (context.stats) {
    const statParts: string[] = [];
    if (context.stats.studyStreak) statParts.push(`${context.stats.studyStreak} day streak`);
    if (context.stats.quizzesCompleted) statParts.push(`${context.stats.quizzesCompleted} quizzes completed`);
    if (context.stats.averageScore) statParts.push(`${context.stats.averageScore}% average score`);
    if (statParts.length > 0) {
      parts.push(`My stats: ${statParts.join(", ")}`);
    }
  }

  if (context.flashcards) {
    parts.push(`Flashcards: ${context.flashcards.total} total, ${context.flashcards.dueToday} due today`);
  }

  if (context.achievements && context.achievements.some(a => a.unlocked)) {
    const unlocked = context.achievements.filter(a => a.unlocked);
    parts.push(`Achievements unlocked: ${unlocked.map(a => a.title).join(", ")}`);
  }
