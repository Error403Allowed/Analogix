...
const VISUAL_INTENT_RE = /\b(graphs?|plots?|charts?|visuali[sz]e|visuali[sz]ation|visuals?|diagrams?|3d|timeline|render|sketch)\b/i;
...
` : ""}`
  ...
  3. CONCEPT DIAGRAMS (for structures & relationships):
   When explaining abstract concepts, structures, systems, or relationships, generate a concept diagram using a JSON code block with language "concept".
...
- CONCEPT DIAGRAMS: For complex concepts, structures, systems, or relationships (e.g. solar system, atomic structure, biological processes, networks), use the concept format described at the end of this prompt to create a visual diagram that illustrates the concept.
...
${visualisationGuide}
IMPORTANT: If the user asks for a visual, diagram, or graph - use the right tool. Math functions → Desmos. Data/statistics → Recharts. Concepts/structures → Concept diagram. Don't just describe it - SHOW it.
- Analogix`;
};