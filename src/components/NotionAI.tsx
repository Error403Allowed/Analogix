...
export function AICommandPalette({ editor, subject, position, query, focused, onFocusedChange, onSelect, onClose }: CommandPaletteProps) {
  ...
  const isInTable = editor.isActive("table") || editor.isActive("tableRow") || editor.isActive("tableCell") || editor.isActive("tableHeader");
  const visibleTableEditItems = query ? tableEditItems : tableEditItems.filter(() => isInTable);

  // Slash menu: only show formatting options, not AI actions
  const allItems: CommandItem[] = [
    ...
    // AI items removed from slash menu - use text selection toolbar instead
  ];
