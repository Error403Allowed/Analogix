...
const FormulaPanel = ({
  ...
  const sheet = selectedSubject ? getFormulaSheet(selectedSubject) : null;
  const topics = sheet ? [...new Set(sheet.formulas.map(f => f.topic))] : [];
  ...
  const body = (
    <div className="flex-1 overflow-y-auto p-2 space-y-1 formula-scroll">
      {(() => {
        if (!sheet) return <p className="text-xs text-muted-foreground/60 text-center py-4">No formula sheet for this subject.</p>;
  ...
  if (isMobile) {
    return (
      <Drawer open={formulaPanelOpen && !!sheet} onOpenChange={setFormulaPanelOpen}>
        <DrawerContent>
          <DrawerHeader>
            <DrawerTitle>{sheet ? `${sheet.label} Formulas` : "Formulas"}</DrawerTitle>
          </DrawerHeader>
          {list}
          {body}
        </DrawerContent>
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {formulaPanelOpen && selectedSubject && sheet && (
          <motion.div
            key="formula-panel"
            ...
            className="hidden md:flex flex-col border border-border rounded-xl bg-card overflow-hidden shrink-0 max-h-[calc(100vh-180px)]"
            style={{ width: 280 }}
          >
