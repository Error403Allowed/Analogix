...
const ConceptDiagram = dynamic(() => import("@/components/graphing/ConceptDiagram"), {
  ssr: false,
  loading: () => (
    <div className="my-4 rounded-2xl overflow-hidden border border-border/30 bg-muted/20 p-6 flex items-center justify-center gap-2">
      <div className="w-1.5 h-1.5 rounded-full bg-primary/40 animate-pulse" />
      <p className="text-xs text-muted-foreground/50 italic">Diagram rendering…</p>
    </div>
  ),
});

import type { ChartSpec, ChartDataPoint } from "@/components/graphing/RechartsGraph";
import type { ConceptDiagramSpec } from "@/components/graphing/ConceptDiagram";
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          pre: ({ children }) => {
            ...
            // Check for concept-diagram block (SVG-based structure/concept visual).
            // Legacy "three"/"3d"/"scene" fences map to the same renderer.
            if (
              lang === "language-concept" ||
              lang === "language-three" ||
              lang === "language-3d" ||
              lang === "language-scene"
            ) {
              if (!blockComplete) return streamingPlaceholder;
              try {
                const spec = JSON.parse(rawBlock) as ConceptDiagramSpec;
                return <ConceptDiagram spec={spec} />;
              } catch {
                return (
                  <pre className="bg-muted/40 border border-border/40 rounded-xl p-4 overflow-x-auto text-sm my-4 select-text">
                    {children}
