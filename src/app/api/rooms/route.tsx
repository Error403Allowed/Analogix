...
export async function GET() {
    try {
        ...
        const membershipMap = new Map((memberships as MembershipRow[] | null ?? []).map((row) => [String(row.room_id), String(row.role)]));
        const mapped = (rooms as Array<{ owner_user_id: string; id: string; visibility: string }> | null ?? []).map((room) => {
            const viewerRole = String(room.owner_user_id) === user.id
                ? "host"
                : (membershipMap.get(String(room.id)) ?? null);
            return mapStudyRoom(room, viewerRole, String(room.owner_user_id) === user.id);
        });
...
export async function POST(request: Request) {
    try {
        ...
        if (canvasError)
            throw canvasError;
        return NextResponse.json({
            room: mapStudyRoom(room, "host", true),
        });
    }
    catch (error) {
