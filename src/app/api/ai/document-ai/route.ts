...
export async function POST(request: Request) {
  try {
    ...
    const { text: content } = await runText({
      system: `You are Analogix Document AI, an educational writing assistant for student notes.${subject ? ` The subject is ${subject}.` : ""} ${formatHint}`,
      prompt,
      taskType: "reasoning",
      temperature: 0.45,
      maxTokens: 8192,
    ...
    console.error("[/api/ai/document-ai] Error:", error);
    return NextResponse.json(
