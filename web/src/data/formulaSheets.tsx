...
if (process.env.NODE_ENV === "development") {
    FORMULA_SHEETS.forEach((sheet) => {
        console.log(`[FormulaSheets] ${sheet.subjectId}: ${sheet.formulas.length} formulas (>=100 guaranteed)`);
    });
}
export const getFormulaSheet = (subjectId) => FORMULA_SHEETS.find(s => s.subjectId === subjectId) || null;
export const getFormulaSheetContext = (subjectId) => {
    ...
    const lines = sheet.formulas.map(f => {
        ...
        if (f.states)
            parts.push(`states: ${f.states.join(",")}`);
        if (f.tags)
...
//# sourceMappingURL=formulaSheets.js.map